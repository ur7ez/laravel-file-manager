<template>
    ...
    <ConfirmationDialog :show="showConfirmationDialog"
                        :message="`Are you sure you want to delete ${props.selectedIds.length} selected item(s)?`"
                        @close="onCancel"
                        @cancel="onCancel"
                        @confirm="onConfirm">
    </ConfirmationDialog>
...
function onConfirm() {
    ...
    form.delete(route('file.deleteForever'), {
        onSuccess: () => {
            showConfirmationDialog.value = false;
            emit('delete');
            showSuccessNotification('Selected items have been deleted forever');
        }
    });
}
